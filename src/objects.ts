...
export function toMarkdown(question: Question): string {
   return [`# ${question.name}`,question.body,...(question.type === "multiple_choice_question" ?question.options.map((option) => `- ${option}`):[]),].join("\n");
}
...
export function mergeQuestion(
    ...
    return {id, name, body: contentQuestion.body,type: contentQuestion.type, options: [...contentQuestion.options],expected: contentQuestion.expected,points, published: false};
}
