...
export function removeQuestion(questions: Question[], id: number): Question[] {
    return questions.filter((ques)=> ques.id !== id);
}
...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.filter((q)=> q.published).reduce((total,q)=> total+q.points,0);
}
...
export function changeQuestionTypeById(
    ...
    return questions.map(q => q.id === targetId ? {...q, type: newQuestionType, options: newQuestionType === "multiple_choice_question"? q.options:[] }:q);
}
    ...
    newOption: string
       ...
       return questions.map((q) => {
           ...
           if (targetOptionIndex === -1) {
               ...
               newOptions = q.options.map((opt, i) =>
...
export function duplicateQuestionInArray(
    ...
    return questions.flatMap(q=> q.id === targetId ? [q, duplicateQuestion(newId, q)]:[q]);
}